...
public class SaiNawKeyboardService extends InputMethodService implements KeyboardView.OnKeyboardActionListener {
    ...
    public void onStartInputView(EditorInfo info, boolean restarting) {
        ...
        if (inputType == EditorInfo.TYPE_CLASS_PHONE) {
            ...
            // IMPORTANT: Don't return here immediately, we need to update Enter label below
        }

        if (keyboardView != null) {
            // Re-apply keyboard just in case (e.g. if we set numberKeyboard manually)
    ...
    // *** NEW: Helper to Change Enter Key Label (Next, Done, Go, etc.) ***
    private void updateEnterKeyLabel(EditorInfo info) {
        ...
        for (Keyboard.Key key : keys) {
            if (key.codes[0] == CODE_ENTER) {
                ...
                key.icon = null; // Remove icon to show label clearly
