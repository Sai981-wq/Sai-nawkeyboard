...
public class SaiNawKeyboardService extends InputMethodService implements KeyboardView.OnKeyboardActionListener {
    ...
    // --- RESTORED ACCESSIBILITY / LIFT-TO-TYPE LOGIC ---
    // This is the code you wanted to keep for Screen Reader users
    ...
    // --- MAIN INPUT HANDLING (MODIFIED FOR THWAY HTOE FIX) ---
    private void handleInput(int primaryCode, Keyboard.Key key) {
        ...
        try {
            switch (primaryCode) {
                case CODE_DELETE:
                    // REMOVED ZWSP LOGIC - Just clean delete
                ...
                default:
                    if (isShanOrMyanmar()) {
                        ...
                        // --- NEW LOGIC: SMART REORDERING WITHOUT ZWSP ---
                        
                        // Check if we need to swap ('ေ' followed by Consonant)
                        boolean swapped = handleSmartReordering(ic, primaryCode);
                        
                        if (swapped) {
                             // Internal Buffer Update for Swapped Chars
                             if (currentWord.length() > 0) {
    ...
    // --- CORE LOGIC: SMART REORDERING (ZWSP REMOVED) ---
    private boolean handleSmartReordering(InputConnection ic, int primaryCode) {
        ...
        // RULE 2: Sort Medials (e.g. 'ှ' before 'ြ')
