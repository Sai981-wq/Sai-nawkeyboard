...
public class SaiNawTouchHandler {
    ...
    public void handleHover(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_HOVER_MOVE:
                int newKeyIndex = getNearestKeyIndexFast(x, y);
            ...
            case MotionEvent.ACTION_HOVER_EXIT:
                if (!isLongPressHandled && lastHoverKeyIndex != -1) {
                    // Ensure we are within a valid state before typing
    ...
    private void scheduleLongPress(Keyboard.Key key) {
        int code = key.codes[0];
        if (code == 32) {
            handler.postDelayed(spaceLongPressTask, 1500);
        } else if (code == -5) {
            handler.postDelayed(deleteStartTask, 1200);
        } else if (code == -1) {
            handler.postDelayed(shiftLongPressTask, 1200);
        } else {
    ...
    private int getNearestKeyIndexFast(int x, int y) {
        ...
        // Optimistic check: is it the same key as before?
        if (lastHoverKeyIndex >= 0 && lastHoverKeyIndex < keys.size()) {
            if (keys.get(lastHoverKeyIndex).isInside(x, y)) {
