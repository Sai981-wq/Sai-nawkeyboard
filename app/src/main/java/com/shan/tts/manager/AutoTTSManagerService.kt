...
class AutoTTSManagerService : TextToSpeechService() {
    ...
    override fun onSynthesizeText(request: SynthesisRequest?, callback: SynthesisCallback?) {
        ...
        // AppLogger.log("Request: ${text.take(20)}...") 
        ...
        try {
            ...
            currentPipeTask = pipeExecutor.submit {
                ...
                try {
                    ...
                    // --- FIX IS HERE: REDUCED BUFFER SIZE ---
                    // 16384 bytes -> 2048 bytes
                    // This prevents the output from becoming too large when upsampling.
                    ...
                    while (!isStopped.get() && !Thread.currentThread().isInterrupted) {
                        ...
                        if (read > 0) {
                            ...
                            if (out.isNotEmpty()) {
                                synchronized(callback) {
                                    try {
                                        // --- EXTRA SAFETY: CHUNK SPLITTER ---
                                        // Even if output is large, send it in small 4KB chunks
                                        var offset = 0
                                        ...
                                        AppLogger.error("Audio write failed (Size: ${out.size})", e)
                                    }
                                }
                            }
                        }
